...
| [`simulator_in_the_loop.ipynb`](simulator_in_the_loop.ipynb) | **Real-world case study** — training against a **non-differentiable simulator** (actuator with deadzone/saturation/quantization); AdaSwarm optimises the true black box where the standard smooth-surrogate approach is biased (~20× lower true error). | Your loss runs a simulator you can't backprop through. |
| [`f0_pitch_estimation.ipynb`](f0_pitch_estimation.ipynb) | **Benchmark vs a specialised method** — audio pitch (F0) estimation where naive Adam collapses into octave errors; `SwarmLoss(per_sample=True)` removes them and **matches pYIN** (`librosa`) with no domain engineering. Includes a **compute/memory cost** section. | You want an honest head-to-head against a real baseline, not just plain Adam. |
